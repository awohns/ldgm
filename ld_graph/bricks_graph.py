...
class BrickGraph:
    ...
    def rule_one(self, edge, children, node_edge_dict, roots):
        # Rule 1: connect parent brick to its child bricks, then to its parent
    ...
    def make_connections(self, edge, tree2, node_edge_dict, index, prev_edge_dict):
        ...
        if self.use_rule_two:
            self.rule_two(edge, children, node_edge_dict)
        if index != 0:
            self.rule_three(edge, children, node_edge_dict, prev_edge_dict)
